//
//  UIDevice+XMAFNetworkingMethods.m
//  XMAFNetworkExample
//
...
@implementation UIDevice (XMAFNetworkingMethods)
...
- (NSString *)XMAF_uuid
{
    ...
    if (uuid.length == 0) {
        [[NSUserDefaults standardUserDefaults] setObject:[self XMAF_createUUID] forKey:key];
...
- (NSString *) XMAF_udid
{
    NSString *udid = [[XMAFUDIDGenerator sharedInstance] UDID];
    if (udid.length==0) {
        udid = [self XMAF_uuid];
        [[XMAFUDIDGenerator sharedInstance] saveUDID:udid];
    }
    return udid;
}
...
- (NSString *) XMAF_macaddressMD5{
    ...
    if (macid.length == 0) {
        NSString *macaddress = [[UIDevice currentDevice] XMAF_macaddress];
        macid = [macaddress XMAF_md5];
        if (!macid){
